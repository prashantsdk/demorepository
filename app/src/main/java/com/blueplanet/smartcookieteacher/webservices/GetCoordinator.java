 ...
 */
public class GetCoordinator extends SmartCookieTeacherService {
    ...
    public GetCoordinator(String t_id, String studentId, int inputId) {
        _t_id = t_id;
    ...
    protected String formRequest() {
        return WebserviceConstants.HTTP_BASE_URL +
                WebserviceConstants.BASE_URL + WebserviceConstants.STUDENT_COORDINATOR_WEB_SERVICE;
    }
    ...
    protected void parseResponse(String responseJSONString) {
        ...
        CoordinatorModel coordinator = null;
        ArrayList<CoordinatorModel> coordinatorList = new ArrayList<>();
        try {
            ...
            if (statusCode == HTTPConstants.HTTP_COMM_SUCCESS) {
                ...
                for (int i = 0; i < responseData.length(); i++) {
                    ...
                    String stuCoordinator = jsonObject.optString(WebserviceConstants.KEY_COORDINATOR);

                    coordinator = new CoordinatorModel(sId, sName, sFname, sSchoolName, sClassName, tMName, SGender,
                            Sdob, sAge, Scity, sEmail, sPRN, sSchoolId, sDate, sDiv, sHobbies, sCountry,
                            sTeacherName, sImagePath, inputId, totalStudentCount,ssubcode,subname,stuCoordinator);
                    coordinatorList.add(coordinator);


                }
                responseObject = new ServerResponse(errorCode, coordinatorList);
            } else {
                // failure
    ...
    public void fireEvent(Object eventObject) {
        ...
        notifier.eventNotify(EventTypes.EVENT_COORDINATOR, eventObject);
    }
}
